...
convenient methods to extract frequencies and time slices of interest--without loading the full file into memory-- 
which are presented as ``Numpy`` arrays [@Numpy:2011]. Methods for manipulating lower-level voltage data products
...
the ``H5py`` package to interact with data stored in HDF5 files [@H5py:2013]. 
