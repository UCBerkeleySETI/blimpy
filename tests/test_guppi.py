...
def test_guppi():
    ...
    _, data_block_x1, data_block_y1 = gr.read_next_data_block_int8()
    _, data_block_x2, data_block_y2 = gr.read_next_data_block_int8()

    assert not np.array_equal(data_block_x1, data_block_x2) \
        and not np.array_equal(data_block_y1, data_block_y2) \
        , "test_guppi: Data read from two blocks should not be equal"

    gr = bl.guppi.GuppiRaw(voyager_raw)
    _, data_block_1 = gr.read_next_data_block()
    ...
    data_block_casted_1 = np.append(data_block_x1,
                                    data_block_y1, axis=2).astype('float32').view('complex64')
